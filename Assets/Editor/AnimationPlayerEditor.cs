...
namespace kmty.anim {
    ...
    public class AnimationPlayerEditor : Editor {
        ...
        private bool isAllCheck {
            ...
            set {
                if (_isAllCheck != value) SetIsEnable(value);
                _isAllCheck = value;
            }
        }
        ...
        List<AnimationPlaylistItem> GetAnimatorStateList(Animator animator) {
            ...
            if (GUILayout.Button("Update Animation Playlist")) {
                ...
                for (int i = 0; i < animatorStates.Count; i++) {
                    ...
                    for (int j = 0; j < copied.Length; j++) {
                        if (state.name.Equals(copied[j].name)) { copied[j].CopyTo(state); break; }
                    }
